            ...
            if (MTME == TRUE){
              ...
              stack_predictions_cv <- function(
                ...
                if (is.null(row.names(Y.raw))) {
                  stop("Y.raw must have row.names set to Taxa / PlantID")
                }
                ...
                ## -----------------------------
                ## 2. Collect valid OOF predictions
                ## -----------------------------
                pred_list <- list(
                ...
                pred_list <- Filter(
                  function(x) is.data.frame(x) && nrow(x) > 0 && ncol(x) > 0,
                  pred_list
                )
